...
def worker_func(worker_id, filepath, input_queue, res_queue, device):
    ...
    agent.policy_job.load_state_dict(torch.load(job_path,map_location='cuda:0'))
    agent.policy_mch.load_state_dict(torch.load(mch_path,map_location='cuda:0'))
...
if __name__ == '__main__':
    ...
    torch.cuda.empty_cache()
    os.environ['CUDA_VISIBLE_DEVICE'] = '0'  # 使用编号为1的显卡
